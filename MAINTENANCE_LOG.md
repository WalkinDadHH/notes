...
Completed:
- 在工程流体力学（甲）Ⅰ讲义前部增加第一次课堂测验速查和目录入口
- 为帕斯卡定理单列章节，补充压强传递、液压机演示题及适用条件
- 按工程热力学的可展开例题格式整理 2026 秋红笔作业题和三道课件窄缝流动题
- 对照甲Ⅰ复习提纲移除甲Ⅱ回忆卷附录、正激波及边界层积分内容；现有甲Ⅰ资料无可核验年份的历年真题

Validation:
- MkDocs 临时目录构建成功；检查帕斯卡、作业及课件例题卡片均在生成页面中
...
- 上线校对后进一步移除甲Ⅱ卷的具体数值题，改用明确标注的演示题，并修复升力公式的表格排版
...
Changed:
- 依据浙大能源工程学院《工程热力学习题册》（2024-2025 第二学期，41 页历年简答+计算题，含 24-25/23-24/22-23/20-21/19-20/19/17 春夏及模拟题）整合「可取部分」
- 全 14 章每章末尾新增 `### 真题精选` 子节，共 66 个 `??? question` 折叠块（简答 + 计算，标题标注学期来源，与既有 `??? example` 例题区分）
- 取舍原则：保留有真实题干的简答与完整数值计算题；合并去重（盖斯定律 4→1、干/湿球/露点三温比较、蒸汽 vs 空气制冷 2→1 等）；删除 3 道「参数同百度文库/类似例 9-1」占位题，但把其通用方法救回为 `!!! tip`（喷管计算通用步骤、回热器两种形式与乏汽干度）；附录 A 物理常数未收（笔记已有符号速查）
- 顶部「章节概览」与「如何使用本页」说明同步更新，加入真题精选指引
- 题册第 1/16 章（绪论、热力循环工程应用）无考题，对应笔记无新增

Why:
- 该课程考核为约 8 道简答 + 3 道计算大题，历年题是高价值备考素材；按章就近放置便于「学完即自测」
- 标题保持纯文本（沿用本日早些时候的修复约定），避免侧栏目录显示 LaTeX 源码

Validation: `scripts/validate-mkdocs.ps1` 通过（0.97s，仅有与本次无关的 engineering-mechanics.pdf 链接旧告警）
...
Changed:
- 第三章末新增「能量的品位与㶲（有效能）」专节：能量品位三分类、㶲/火无定义、热量㶲（卡诺因子）、闭口系㶲与稳流焓㶲、Gouy–Stodola 㶲损定理（$I = T_0 S_g$）、㶲平衡方程、㶲效率，并附例题 3-2（定容升温的㶲分析：直接加热 vs 可逆热泵）
- 「符号速查」增补有效能符号组：$E_x$、$E_{x,Q}$、$I$、$\eta_{ex}$
- 「章节概览」第三章标题更新为「熵与热力学第二定律（含㶲/有效能分析）」
- 第 9 章补充滞止参数、有摩擦的实际流动；第 11 章补充循环评价指标（汽耗率/循环功比/相对内效率）；第 12 章补充空气标准假设五条；第 13 章新增例题 13-2（空气压缩制冷循环，增压比影响）；第 14 章补充理论燃烧温度、反应方向与最大功判据、热力学第三定律（奈斯特定理），并与第三章㶲概念呼应

Why:
- 㶲分析是工程热力学第二定律的核心应用、能源系统节能的关键视角，原笔记缺该专节
- 后半计算章节补充评价指标与判据，降低阅读与做题门槛

Verified:
- `scripts/validate-mkdocs.ps1` 非破坏式构建（待运行确认）
...
Changed:
- 第三章方法总览表更新：把“某些特殊系数可化为常系数”从原 Liouville（刘维尔）公式附近单独拆成新 `§3.5`
- 原 `§3.5` 常数变易法顺延为 `§3.6`
- `§3.4` 改为“设 `y=uy_1` 降阶”口径，补出 Liouville（刘维尔）公式的推导步骤
- 新增 `§3.5` 特殊系数法的板书式推导与例题 `4y'' + 4xy' + (x^2+1)y = 0`
- `§3.6` 常数变易法改成“先列关于 `u_1',u_2'` 的方程组，再直接消元”口径，并新增一题课堂板书风格例题
- 同步重写现有常数变易例题步骤，去掉例题中不必要的 Wronskian（朗斯基）行列式展开；其中 `(x-2)y'' + 2y' - (x-2)y = x^2-2x-2` 的特解改正为 `y^*=-x`
...
变更：大幅扩充各章例题的推导步骤，降低阅读门槛：

- §1.1 例题：补充偏分式分解过程，展示 $\int\mathrm{d}y/(1-y^2)$ 的完整计算，并说明遗漏解检查
- §1.2 例题：补充分离变量、换元积分、回代 $u=y/x$、代初值的逐步推导
- §1.3：新增完整例题（$y'-2y/x=x^2$，含积分因子推导与初值代入）
- §1.4 例题：展开 $z=y^{1/2}$ 的代换推导、积分因子 $x^{-2}$ 的求法、积分过程及回代
- §1.5 例题：补充特解验证（$a^2-a-2=0$）、代换展开（$u=y-2/x$）、Bernoulli→线性的逐步推导、线性方程求解全程
- §1.8 例题1：补全互换变量后的线性方程完整求解（积分因子 $y^2$、积分、回代）
- §2.1 例题：展开对 $(12x^2-6)e^{-x}$ 的分部积分两步，补充两次积分求 $y$ 的过程
- §2.2 例题1：展开积分因子求解、两次初值代入，给出最终显式解 $y=e^{2(e^x-1)}$
- §3.2 例题1：展开 ${y^*}''$ 的计算，逐项比较 $\cos 2x$、$\sin 2x$ 系数，确定 $A=B=-1/8$

## 2026-04-19 - ODE 笔记 2.x / 3.x 勘误（对照 Savia 第 2、3 讲）

Scope: `docs/math/ode/index.md` 第二章、第三章

Fixed:
- §2.1 例题（20-21）：方程阶数错误，`y'' - y' = 12x²-6` 改为三阶 `y''' - y'' = 12x²-6`；解改为两次积分，答案 `y = -x⁴ - 4x³ - 9x² + C₁eˣ + C₂x + C₃`
- §3.1 例题（23-24 春）：方程系数符号全错，`y⁴+2y'''+3y''+2y'+y=0` 改为 `y⁴-2y'''+3y''-2y'+y=0`；特征根 α 从 -1/2 改为 +1/2；通解由 e^(-x/2) 改为 e^(x/2)

## 2026-04-18 - ODE 笔记 4.x 勘误（对照 Savia 第 4 讲）

Scope:
- `docs/math/ode/index.md` 第四章内容勘误
- 依据 `docs/《常微分方程》第 4 讲  线性微分方程组.pdf` 完整三页

Fixed:
- 4.1 广义特征向量链递推式方向写反：原 `(A-λE)v_{j+1}=v_j` 改为 `(A-λE)v_{j-1}=v_j`，并补全 `(A-λE)^k v_0 = 0` 的种子步骤与链底为普通特征向量的说明
- 4.1 例题（23-24 夏）矩阵符号错：y、z 两行符号原本写反（`-2y-2z / +4y+2z / -5y-3z` → 正确为 `+2y+2z / -4y-2z / +5y+3z`），导致特征向量 $(1,1,0),(1,1,1),(0,1,1)$ 也全错（正确为 $(1,-1,0),(1,-1,1),(0,-1,1)$），通解中 y 应带负号。已重写为 ①写矩阵求特征值 ②逐个求特征向量 ③组合通解 三步完整过程
- 4.2 例题（23-24 夏）题面、初值、基本解矩阵全部按参考原题重写（`x'=2x+y, y'=x+2y-2e^{2t}, x(0)=y(0)=1`），求解展开为 ①②③ 三步

备注：
- 4.2 例题 2 (24-25) 已对照 `C:/Users/KANEKI/Desktop/24-25春学期常微分方程期末卷.pdf` II.2，题面/特征值/特征向量/特解常向量/通解全部一致，无需改
- 4.3 消元法、4.4 二阶化一阶例题在 Savia 第 4 讲与两份期末回忆卷中均无对应题，本次未改
- 4.x 章节至此已与所有可用参考源（Savia 第 4 讲、23-24 夏回忆卷、24-25 春期末卷）完整比对
