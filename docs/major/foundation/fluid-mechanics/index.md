...
这份手写记录由同学提供，**没有试卷抬头、年份和完整数值**。所记内容对应甲Ⅰ的连续性方程、管路损失、烟囱流动、等熵气流和量纲分析。下列卡片保留原题线索，缺失处用符号推导；条件不足时说明还需要什么，避免把补充假设当成原题。可[展开查看手写原图](#fluid-i-recall-photo)。
...
??? question "年份未注明 · 计算①｜由两截面积求局部阻力系数"
    ...
    **原题线索**：已知 $A_1,A_2$，求 $\zeta_2$。原图没有管件形状，也没有标明 $\zeta_2$ 的参考速度。
...
??? question "年份未注明 · 计算｜烟囱高度与抽力"
    ...
    **原题线索**：烟囱直径 $d$，烟气密度 $\rho_1$，大气密度 $\rho_2$，烟气流量 $Q$，沿程阻力系数 $\lambda$；截面 1 的负压需超过图中所写阈值（手写数值疑为 $150\ \mathrm{Pa}$），求高度 $H$。
...
??? question "年份未注明 · 计算｜两截面等熵气流的速度、温度和马赫数"
    ...
    **原题线索**：已知截面 1 的 $p_1,T_1,A_1$，截面 2 的 $p_2,A_2$，以及气体比热比 $\gamma_g$、气体常数 $R$；求 $V_1,\mathrm{Ma}_1,V_2,T_2,\mathrm{Ma}_2$。图中注明等熵。
...
定常多入口、多出口时，应逐个截面写 $\sum\dot m_{\rm in}=\sum\dot m_{\rm out}$。回忆卷中的[极坐标连续性证明](#fluid-i-recall)是同一个守恒条件在另一组坐标中的表达。
